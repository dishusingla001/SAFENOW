...
const MapView = ({ requests, selectedRequest }) => {
  ...
  const [focusedRequest, setFocusedRequest] = useState(selectedRequest);
  ...
  if (
    ...
    return (
      <div className="w-full h-[400px] bg-dark-800 rounded-lg flex flex-col items-center justify-center p-6 text-center">
  ...
  const center = focusedRequest
    ? {
        lat: focusedRequest.location.latitude,
        lng: focusedRequest.location.longitude,
      }
    : selectedRequest
      ? {
    ...
    <div className="space-y-4">
      <LoadScript googleMapsApiKey={GOOGLE_MAPS_API_KEY}>
        <GoogleMap
          ...
          center={center}
          zoom={focusedRequest || selectedRequest ? 15 : 12}
          options={{
            styles: [
              // Dark theme for map
